...
const {AddTrans} = useContext(GlobalContext)
    return (
        ...
        <br/>
        <TextField id="standard-basic" type="number" label="Amount" onChange={(e)=>(setam(e.target.value))} fullWidth style={{paddingLeft:"10px",width:"430px",marginRight:"10px"}}/>
    <br/><br/>
        <Button disabled={(des === '') || (am === 0)  ? true:false  } variant="contained" onClick={()=>{
            var id =  1 + (Math.random() * (100-1));
